...
- **Kanban module** — three-column drag-and-drop task board (todo / in_progress / done) with filtering and basic stats
- **Work Weekly module** — project-based tasks/work-orders with hour tracking, milestones, saturation stats (40h/week baseline), cross-week splitting, and a dashboard
...
backend/app/
  ...
  models/              # SQLAlchemy ORM models (User, Task, Project, WorkItem, WorkLog, Milestone)
  schemas/             # Pydantic v2 models for request/response validation
  routers/             # 8 route modules (auth, tasks, stats, projects, work_items, work_logs, work_stats, milestones)
...
frontend/src/
  ...
  api/*.ts             # Typed API functions (8 files, one per backend router group)
  ...
  views/
    LoginView.vue        # Login/Register with centered card layout
...
4. **Work week target**: 40 hours/week constant (`WEEKLY_HOURS = 40` in `work_stats.py`)
